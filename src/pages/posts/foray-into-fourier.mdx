...
much before any music visualizers and discovered it with maths. Thats right, 
...
composed of a set of frequencies. For an overview of what I mean by frequency
feel free to check out [my previous blog](./dopplin-around) on the Doppler
Effect where I give a brief explanation.
...
amplitude is how big/strong the wave/frequency is and the phase moves the wave
...
numbers are not very similar at all. Conversely if $a$ **and** $b$ is negative
...
$b = (x_2, y_2)$ then how could we possible tell how similar these coordinates
are to one another?

You see if we think about those coordinates as vectors (just a fancy word for numbers in a row or column) and rewrote them like this: 
$$
a = \begin{bmatrix}
x_1 \\
y_1
...
So this is looking more promising, however, we have to now just juggle two
...
\begin{bmatrix}x_1\\y_1\end{bmatrix}\cdot\begin{bmatrix}x_2\\y_2\end{bmatrix} = (x_1\times
x_2) + (y_1\times y_2)
$$
...
procedure. We multiply the corresponding dimension (ie. the $x$'s multply and
the $y$'s multiply) and then add up the result.

For example:
$$
a \cdot b = \begin{bmatrix}x_1\\y_1\\z_1 \\ \vdots\\ N_1 \end{bmatrix} \cdot
\begin{bmatrix}x_2\\y_2\\z_2 \\ \vdots\\ N_2 \end{bmatrix} = (x_1 \times x_2) +
(y_1 \times y_2) + (z_1 \times z_2) + \dots + (N_1 \times N_2) 
$$
...
then compute the function as the sine of that number. Here are a few examples:
$$
f(0.1) = \sin(0.1)\\
f(-0.1) = \sin(-0.1)\\
f(10) = \sin(10)\\
...
a row for 0.1, another row for 0.001, antother for 0.0000001 etc.
...
$$
\begin{split}
f(t) \cdot g(t) = & (f(-\infin)\times g(-\infin)) + \dots \\
&+ (f(-0.0001)\times g(-0.0001)) + \dots \\
& + (f(0)\times g(0)) + \dots + (f(0.0001)\times g(0.0001)) \\
& + \dots + (f(\infin)\times g(\infin))
\end{split}
$$
...
So this means out dot product or similarity of two functions can be written as:
$$
...
just there to convert the answer into Hertz. Fun fact, this is exactly where
the famous equation $e^{i\pi} = -1$ comes from.
...
This is because the magic of Eulers equation and complex numbers in general
allows for visualizing $e^{-j2\pi ft}$ as a rotating vector on the complex
plane. An example of this can be seen in the animation below: 
...
is instead of starting on the x-axis it could start at a different point on the
circle.
...
Summary:
- The term $e^{-j2\pi ft}$ is a function that represents a spinning vector/line with a given frequency, amplitude and phase.
...
So to sum it all up, the Fourier Transform computes how similar our signal $f(t)$ is to the frequency, and beyond that it computes the amount of similarity between our signal and the frequency $f$. So bringing it back to the equation: 
$$
\mathcal{F}(f) = \int^{\infin}_{-\infin}f(t)e^{-j2\pi ft} dt 
$$
...
complete with phase and amplitude information.
...
would need to do one million calculations. This is known as an $O(n^2)$
...
together elements of complex numbers, Eulers formula and cross products. The
...
The idea of visulizing each frequency of a signal as a rotating vector allows
...
compute all of the frequencies in Henry (the image, the dog is far more
...
TODO: Fourier Drawing 

Note that you can adjust how many frequencies are drawn using the slider. Have
a play with it and see how the less frequencies we include the less accurately
the drawing represents the Henry signal.
